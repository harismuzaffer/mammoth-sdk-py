"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from mammoth_analytics.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from pydantic import model_serializer
from typing_extensions import NotRequired, TypedDict


class CloudDsSnowflakeBatchSourceTypedDict(TypedDict):
    database: NotRequired[Nullable[str]]
    r"""Database Name"""
    host: NotRequired[Nullable[str]]
    r"""Host"""
    username: NotRequired[Nullable[str]]
    r"""User name"""
    account: NotRequired[Nullable[str]]
    r"""Account"""
    warehouse: NotRequired[Nullable[str]]
    r"""Warehouse"""
    role: NotRequired[Nullable[str]]
    r"""Role"""


class CloudDsSnowflakeBatchSource(BaseModel):
    database: OptionalNullable[str] = UNSET
    r"""Database Name"""

    host: OptionalNullable[str] = UNSET
    r"""Host"""

    username: OptionalNullable[str] = UNSET
    r"""User name"""

    account: OptionalNullable[str] = UNSET
    r"""Account"""

    warehouse: OptionalNullable[str] = UNSET
    r"""Warehouse"""

    role: OptionalNullable[str] = UNSET
    r"""Role"""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = [
            "database",
            "host",
            "username",
            "account",
            "warehouse",
            "role",
        ]
        nullable_fields = [
            "database",
            "host",
            "username",
            "account",
            "warehouse",
            "role",
        ]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)
            serialized.pop(k, None)

            optional_nullable = k in optional_fields and k in nullable_fields
            is_set = (
                self.__pydantic_fields_set__.intersection({n})
                or k in null_default_fields
            )  # pylint: disable=no-member

            if val is not None and val != UNSET_SENTINEL:
                m[k] = val
            elif val != UNSET_SENTINEL and (
                not k in optional_fields or (optional_nullable and is_set)
            ):
                m[k] = val

        return m
